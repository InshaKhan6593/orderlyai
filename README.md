...
> `api/.env` is pre-filled for local dev. Change `JWT_SECRET` before deploying.

## Tests

```bash
cd api
uv run pytest            # 32 tests: auth, tenant isolation, validation, enums, CRUD, pricing/status
...
to be a member of that business.
...
- **Status machine:** `pending → accepted → preparing → ready/out_for_delivery →
  completed` (+ `rejected`/`cancelled`); invalid jumps are rejected.
